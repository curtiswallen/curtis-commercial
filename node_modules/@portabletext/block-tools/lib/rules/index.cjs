...
var schema = require("@portabletext/schema"), helpers = require("../_chunks-cjs/helpers.cjs");
    ...
    deserialize: (node, next) => {
      ...
      for (const row of bodyRows) {
        ...
        for (const cell of cells) {
          ...
          const flattenedHeaderResult = helpers.flattenNestedBlocks(
            { schema: schema$1 },
            Array.isArray(headerResult) ? headerResult : [headerResult]
          ), firstFlattenedHeaderResult = flattenedHeaderResult[0], flattenedResult = helpers.flattenNestedBlocks(
            { schema: schema$1 },
            Array.isArray(result) ? result : [result]
          ), firstFlattenedResult = flattenedResult[0];
          if (flattenedHeaderResult.length === 1 && schema.isTextBlock({ schema: schema$1 }, firstFlattenedHeaderResult) && flattenedResult.length === 1 && schema.isTextBlock({ schema: schema$1 }, firstFlattenedResult)) {
...
exports.createFlattenTableRule = createFlattenTableRule;
